...
ngay trong test thay vì phụ thuộc output/catalog_master_entity_resolved.xlsx thật
(41k dòng)."""
...
_LISTING_HEADER = [
    "listing_id", "master_product_id", "variant_id", "source", "product_id",
    "source_url", "drug_name", "nhà_sản_xuất_xuất_xứ",
]


def _write_workbook(path: Path, master_rows: list[tuple], listing_rows: list[tuple]) -> None:
    wb = Workbook()
    wb.remove(wb.active)
...
class TestLoadMasterCatalog:
    ...
    def test_basic_load(self, tmp_path: Path) -> None:
        ...
        _write_workbook(
            ...
            listing_rows=[
                ("L1", "MP1", "PV1", "Giathuoctot", "p1",
                 "https://a.test/p1", "Boganic Forte 100v", "Traphaco"),
                ("L2", "MP1", "PV1", "ChoThuoc247", "p2",
                 "https://b.test/p2", "Boganic H100V", ""),
            ],
        )
        items = load_master_catalog(path)
        assert len(items) == 2
        assert {it.product_id for it in items} == {"p1", "p2"}
        ...
        assert by_id["p1"].source_url == "https://a.test/p1"
        assert by_id["p2"].manufacturer == ""
    ...
    def test_unknown_master_id_falls_back_to_listing_name(self, tmp_path: Path) -> None:
        ...
        _write_workbook(
            ...
            master_rows=[],
            listing_rows=[
                ("L1", "MP_missing", "PV1", "Giathuoctot", "p1",
                 "https://a.test/p1", "Ten Goc", ""),
            ],
        )
        items = load_master_catalog(path)
        assert len(items) == 1
    ...
    def test_skips_row_with_invalid_source(self, tmp_path: Path) -> None:
        ...
        _write_workbook(
            ...
            listing_rows=[
                ...
                ("L2", "MP1", "PV1", "Giathuoctot", "p2", "https://b.test/p2", "Y", ""),
            ],
        )
        items = load_master_catalog(path)
        assert len(items) == 1
    ...
    def test_load_orders_groups_by_canonical_name_instead_of_source_rows(
        ...
        _write_workbook(
            ...
            listing_rows=[
                ("L1", "MP2", "PV2", "BachHoaThuoc", "bht-beta", "", "Beta", ""),
                ("L2", "MP1", "PV1", "BachHoaThuoc", "bht-alpha", "", "Alpha", ""),
                ("L3", "MP1", "PV1", "ThuocHaPu", "hapu-alpha", "", "Alpha", ""),
            ],
        )

        items = load_master_catalog(path)

        assert [item.product_id for item in items] == ["bht-alpha", "hapu-alpha", "bht-beta"]


_FULL_MASTER_HEADER = [
    "master_product_id", "tên_sản_phẩm_chuẩn", "hàm_lượng_chuẩn", "hoạt_chất_mô_tả",
    "dạng_bào_chế", "nhà_sản_xuất_xuất_xứ", "số_listing", "số_nguồn", "các_nguồn",
    "phương_pháp_ghép", "độ_tin_cậy", "trạng_thái",
]
_FULL_LISTING_HEADER = [
    "listing_id", "master_product_id", "variant_id", "source", "product_id",
    "source_url", "drug_name", "loại_bản_ghi", "tên_sản_phẩm_đã_tách", "hàm_lượng",
    "hoạt_chất_mô_tả", "dạng_bào_chế", "nhà_sản_xuất_xuất_xứ", "quy_cách_đóng_gói",
    "pack_signature", "phương_pháp_ghép", "độ_tin_cậy", "cần_duyệt",
]
...
def _ci(product_id: str, source: SourceName, source_url: str = "https://x.test/p") -> CatalogItem:
    return CatalogItem(product_id=product_id, drug_name="tmp", source=source, source_url=source_url)
...
class TestAppendManualProduct:
    def test_appends_master_and_listing_rows(self, tmp_path: Path) -> None:
        ...
        _write_full_workbook(
            path,
            master_rows=[("MP000001", "Sản phẩm cũ", None, None, None, None, 1, 1,
                           "Giathuoctot", "entity-resolution", 0.9, "đã duyệt")],
            listing_rows=[("L0000001", "MP000001", None, "Giathuoctot", "old-p1",
                            "https://a.test/old", "Sản phẩm cũ", "sản phẩm", None,
                            None, None, None, None, None, "entity-resolution", 0.9, "Không")],
        )
        items = [
        ...
        new_listing_ids = [r[1] for r in listing_rows_after[2:]]
        lidx = {c: i for i, c in enumerate(listing_rows_after[0])}
        assert all(r[lidx["master_product_id"]] == "MP000002" for r in listing_rows_after[2:])
        assert {r[lidx["product_id"]] for r in listing_rows_after[2:]} == {"new-p1", "new-p2"}
    ...
    def test_preserves_existing_rows_untouched(self, tmp_path: Path) -> None:
        ...
        _write_full_workbook(
            path,
            master_rows=[("MP000001", "Cũ 1", None, None, None, None, 1, 1,
                           "Giathuoctot", "entity-resolution", 0.9, "đã duyệt")],
            listing_rows=[("L0000001", "MP000001", None, "Giathuoctot", "old-p1",
                            "https://a.test/old", "Cũ 1", "sản phẩm", None,
                            None, None, None, None, None, "entity-resolution", 0.9, "Không")],
        )
...
class TestAppendOrUpdateListing:
    def test_adds_new_site_bumps_master_counts(self, tmp_path: Path) -> None:
        ...
        _write_full_workbook(
            path,
            master_rows=[("MP000001", "Boganic", None, None, None, None, 1, 1,
                           "Giathuoctot", "entity-resolution", 0.9, "đã duyệt")],
            listing_rows=[("L0000001", "MP000001", None, "Giathuoctot", "p1",
                            "https://a.test/p1", "Boganic", "sản phẩm", None,
                            None, None, None, None, None, "entity-resolution", 0.9, "Không")],
        )
    ...
    def test_updates_existing_site_in_place_no_master_bump(self, tmp_path: Path) -> None:
        path = tmp_path / "catalog.xlsx"
        _write_full_workbook(
            path,
            master_rows=[("MP000001", "Boganic", None, None, None, None, 1, 1,
                           "Giathuoctot", "entity-resolution", 0.9, "đã duyệt")],
            listing_rows=[("L0000001", "MP000001", None, "Giathuoctot", "old-id",
                            "https://a.test/old", "Boganic", "sản phẩm", None,
                            None, None, None, None, None, "entity-resolution", 0.9, "Không")],
        )
        updated_item = _ci("new-id", SourceName.GIATHUOCTOT, "https://a.test/updated")
        append_or_update_listing("MP000001", updated_item, "Boganic", path=path)
        ...
        lidx = {c: i for i, c in enumerate(listing_rows[0])}
        assert listing_rows[1][lidx["listing_id"]] == "L0000001"  # giữ nguyên listing_id
        assert listing_rows[1][lidx["product_id"]] == "new-id"
        assert listing_rows[1][lidx["source_url"]] == "https://a.test/updated"

        midx = {c: i for i, c in enumerate(master_rows[0])}
        assert master_rows[1][midx["số_listing"]] == 1  # không tăng vì chỉ SỬA, không thêm site mới
    ...
    def test_no_duplicate_source_name_in_cac_nguon(self, tmp_path: Path) -> None:
        ...
        append_or_update_listing(
            "MP000001", _ci("p2", SourceName.CHOTHUOC247, "https://b.test/p2"), "Boganic", path=path
        )
        append_or_update_listing(
            "MP000001", _ci("p2-v2", SourceName.CHOTHUOC247, "https://b.test/p2v2"), "Boganic", path=path
        )
        ...
        midx = {c: i for i, c in enumerate(master_rows[0])}
        assert master_rows[1][midx["số_nguồn"]] == 2  # Giathuoctot + ChoThuoc247, không đếm 2 lần
        assert master_rows[1][midx["các_nguồn"]] == "ChoThuoc247; Giathuoctot"
        assert len(listing_rows) == 3  # header + Giathuoctot cũ + ChoThuoc247 (sửa tại chỗ lần 2)
